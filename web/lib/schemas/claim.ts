...
const substituteRentalSchema = z.object({
...
const genericProcessStepSchema = z
  .object({
    title: z.string(),
    status: z.string(),
  })
...
export const processDetailSchema = z.union([
  towingServiceSchema,
  claimNotificationSchema,
  appraisalSchema,
  substituteRentalSchema,
  fileReviewSchema,
  deductionReasonSchema,
  paymentInformationSchema,
  closedSchema,
  genericProcessStepSchema,
]);

export const claimDashboardSchema = z.object({
  title: z.string(),
  fileNo: z.string(),
...
export type ClaimDashboard = z.infer<typeof claimDashboardSchema>;
